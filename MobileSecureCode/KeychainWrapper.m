...
+ (NSString *)securedSHA256DigestHashForPIN:(NSUInteger)pinHash
{
    ...
    int hash = [CommonFunc ComputeHash:obfuscatedSecretKey];
    NSLog(@"[Server ComputeHash:%d", hash);
    NSString *computedHashString = [NSString stringWithFormat:@"%@%lu%@%d", name, (unsigned long)pinHash, SALT_HASH, hash];
    // 3
